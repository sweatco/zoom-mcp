/**
 * VTT parsing for the proxy.
 *
 * This mirrors `src/utils/vtt-parser.ts` in the MCP client so that a transcript
 * reads identically whether it was fetched directly (meetings the caller hosted)
 * or through this proxy (meetings they only attended). The two used to diverge:
 * the proxy dropped timestamps, the client kept them. Keep them in sync.
 */
